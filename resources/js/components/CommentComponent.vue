    ...
    <div class="card mt-3" v-for="comment in comments" :key="comment.id">
      <div class="card-body d-flex align-items-start">
        <a :href="'/users/'+comment.user.name" class="text-dark mr-3">
          <img :src="comment.user.image ? comment.user.image : '/images/noimage01.png'" class="rounded-circle" style="width: 50px; height: 50px; object-fit: cover;">
        </a>
        <div class="w-100">
          <div class="d-flex justify-content-between align-items-start w-100">
            <div>
              <div class="font-weight-bold">
                <a :href="'/users/'+comment.user.name" class="text-dark">
...
export default {
  ...
  methods: {
    ...
    deleteComment(commentId) {
      axios.patch(`/comment/delete/${commentId}`, {})
        .then(response => {
    ...
    fetchComments() {
      axios.get(`/api/article/${this.article_id}/comments`)
        .then(response => {
